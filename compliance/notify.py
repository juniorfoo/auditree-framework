...
from pymsteams import TeamsWebhookException, cardsection, connectorcard
...
class TeamsNotifier(_BaseNotifier):
    """
    Teams notifier class.

    Notifications are sent to Teams channel(s).  This notifier is
    configurable via :class:`compliance.config.ComplianceConfig`.
    """
    ...
    def __init__(self, results, controls, push_error=False):
        """
        Construct and initialize the Teams notifier object.

        :param results: dictionary generated by
          :py:class:`compliance.runners.CheckMode` at the end of the execution.
        :param controls: the control descriptor that manages accreditations.
        """
        super(TeamsNotifier, self).__init__(results, controls, push_error)
        self._creds = get_config().creds
        self._config = get_config().get('notify.teams', {})

    def notify(self):
        """Send notifications to Teams channel(s)."""
        self.logger.info('Running the Teams notifier...')
        if not self._config:
            self.logger.warning('Using Teams notifier without config')
        ...
        for accreditation, desc in messages:
            ...
            if isinstance(self._config[accreditation], list):
                ...
                channels = self._config[accreditation].get('channels', [])
                mode = self._config[accreditation].get('mode', 'normal')
            card = self._generate_card(accreditation, desc, mode=mode)
            self._send_card(card, channels)

    def _generate_card(self, accreditation, test_descs, mode='normal'):
        if not test_descs:
            return {}

        # Create our teams message with a bogus webhook
        card: connectorcard = connectorcard('bogus')
        ...
        modes = {
            'normal': self._generate_normal_section,
            'compact': self._generate_compact_section
        }
        if mode in modes.keys():
            if self._push_error:
                self._generate_push_error_section(card)
            else:
                modes[mode](test_descs, card)
        else:
            raise ValueError(f'Unknown Teams message mode: {mode}')

        return card

    def _generate_push_error_section(self, card: connectorcard):
        section = cardsection()
        section.title('ALL checks')
        section.text(
            'Evidence/Results failed to push to remote locker. '
            'See execution log for details.'
        )

        card.color(TeamsNotifier.MESSAGE_COLORS['error'])
        card.addSection(section)

    def _generate_normal_section(self, test_descs, card: connectorcard):
        passed, failed, warned, errored = self._split_by_status(test_descs)

        if len(errored) > 0:
            card.color(TeamsNotifier.MESSAGE_COLORS['error'])
        elif len(failed):
            card.color(TeamsNotifier.MESSAGE_COLORS['fail'])
        elif len(warned):
            card.color(TeamsNotifier.MESSAGE_COLORS['warn'])
        elif len(warned):
            card.color(TeamsNotifier.MESSAGE_COLORS['pass'])
        ...
        section.text(', '.join(passed_titles))
        card.addSection(section)

    def _generate_compact_section(self, test_descs, card: connectorcard):
        passed, failed, warned, errored = self._split_by_status(test_descs)
        ...
        if passed:
            section = cardsection()
            section.title(f'PASS: {len(passed)} checks')
            section.text('')
            card.addSection(section)
        ...
        for status, c in collections:
            text = ''
            for _, test_desc, msg in c:
                summary, body = self._get_summary_and_body(test_desc,
                                                           msg,
                                                           include_title=False,
                                                           link_format=lnk_fmt,
                                                           summary_format=fmt)
            ...
            if text:
                section = cardsection()
                section.title(f'{status.upper()}: {len(c)} checks')
        ...
        if errored:
            section = cardsection()
            section.title(f'ERRORS: {len(errored)} checks')
            section.text(', '.join(set(self._get_check_names(errored))))
            card.addSection(section)

    def _send_card(self, card: connectorcard, channels):
        message: connectorcard = copy.deepcopy(card)
        for c in channels:
            url = getattr(self._creds['teams'], f'{c}_webhook')
            if not url:
                raise RuntimeError(
                    'Unable to get a Teams webhook or token from '
                    'credentials file'
                )
            message.newhookurl(url)

            retries = self._config.get('retries', 4)
            retry = 1
            while retry < retries:
                try:
                    message.printme()
                    ...
                    # Please note that response body needs to be extracted and
                    # read as Connectors do not throw 429s
